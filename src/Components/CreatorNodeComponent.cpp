...
void Ogreman::CreatorNodeComponent::SpecificInitComponent(VeryReal::Component* c) {
	...
	float cost,hcost;
	int id;
	if (std::holds_alternative<std::string>(parameters_map.at("ispatrol")->GetVariant())) {
		ispatrol = std::get<bool>(parameters_map.at("ispatrol")->GetVariant());
	}
	if (std::holds_alternative<std::string>(parameters_map.at("iswalkable")->GetVariant())) {
		iswalkable = std::get<bool>(parameters_map.at("name")->GetVariant());
	}
	if (std::holds_alternative<std::string>(parameters_map.at("cost")->GetVariant())) {
		cost = std::get<float>(parameters_map.at("cost")->GetVariant());
	}
	if (std::holds_alternative<std::string>(parameters_map.at("hcost")->GetVariant())) {
		hcost = std::get<float>(parameters_map.at("hcost")->GetVariant());
	}
	if (std::holds_alternative<std::string>(parameters_map.at("id")->GetVariant())) {
